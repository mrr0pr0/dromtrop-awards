...
export function getCategoryLayout(
	...
	if (
		n.includes('mest originale idé') ||
		n.includes('mest originale ide')
	)
		return 'originalIdea';
	if (
		n.includes('beste interaktiv') ||
		n.includes('beste spill')
	)
		return 'interactive';
	if (n.includes('beste animasjon')) return 'animation';
	if (n.includes('beste historiefortelling'))
		return 'storytelling';
...
export function getVisibleNomineeFields(
	categoryName: string,
): NomineeFieldKey[] {
