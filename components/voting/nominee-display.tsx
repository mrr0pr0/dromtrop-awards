      ...
      >
        {layout === "shortFilm" && nominee.video_url ? (
          ...
          <img
            ...
            alt={nominee.name}
            onClick={layout === "medie" ? onImageClick : undefined}
            className={cn(
              "h-full w-full object-cover",
              layout === "medie" && onImageClick && "cursor-pointer",
            )}
          />
        ) : (
  ...
  if (layout === "medie") {
    return (
      <div className={className}>
        <h3 className="text-lg font-semibold text-gold md:hidden">{nominee.name}</h3>
        {nominee.image_url && (
          <img
            src={nominee.image_url}
            alt={nominee.name}
...
export function nomineeSummaryText(nominee: Nominee): string {
