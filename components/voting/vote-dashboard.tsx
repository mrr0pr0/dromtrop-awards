...
export function VoteDashboard({
  ...
  const totalNominees = useMemo(
    () => Object.values(nominees).reduce((sum, list) => sum + list.length, 0),
    [nominees],
  );
  ...
  async function handleVote(categoryId: number, nomineeId: number) {
    ...
    try {
      ...
      const filtered = localVotes.filter((v) => v.category_id !== categoryId);
      filtered.push({
        id: 0,
      ...
      const nominee = nominees[categoryId]?.find((n) => n.id === nomineeId);
      setToast(
        nominee
  ...
  const visibleCategories = categories.filter((category) => {
    ...
    const searchBlob = `${category.name} ${category.description ?? ""} ${list.map((n) => `${n.name} ${n.description ?? ""}`).join(" ")}`.toLowerCase();
    const matchesText = !query || searchBlob.includes(query);
    ...
    <div className="text-white">
        ...
        >
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            <div className="rounded-[10px] border border-white/15 px-2 py-3 text-center">
              ...
              <span className="mt-1 block text-xs text-white/70">stemmer</span>
            </div>
            <div className="rounded-[10px] border border-white/15 px-2 py-3 text-center">
              <strong className="block text-2xl tabular-nums text-gold">
                {categories.length}
              </strong>
              <span className="mt-1 block text-xs text-white/70">kategorier</span>
            </div>
            <div className="rounded-[10px] border border-white/15 px-2 py-3 text-center">
              <strong className="block text-2xl tabular-nums text-gold">
                {totalNominees}
              </strong>
              <span className="mt-1 block text-xs text-white/70">nominerte</span>
            </div>
          </div>
        </aside>
      </section>
      ...
      >
        <label className="relative block">
          <span className="sr-only">Søk i kategorier og nominerte</span>
          <input
            type="search"
            value={search}
                    ...
                    <div>
                      {list.map((nominee) => (
                        <NomineeListRow
                          ...
                          isSelected={
                            categoryVote?.nominee_id === nominee.id
                          }
                          isVoting={
